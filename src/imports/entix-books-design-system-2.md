...
### Screen 6: Contacts — جهات الاتصال (Party Model)
```
⚠️ ARCHITECTURE: Party Model — identity separate from roles
One record per entity, multiple roles, no duplicates

Route: /contacts

Header: "جهات الاتصال" + "إدارة جميع الأطراف ذات العلاقة"
Action: "إضافة جهة +" button (blue)

Filter Tabs (by ROLE — not by type):
  [الكل] [عملاء 🔵] [موردين 🟢] [موظفين 🟣] [فري لانسر 🟡] [مساهمين 🔵]

Type Toggle: [منظمات 🏢] [أفراد 👤] [الكل]

Search: "بحث بالاسم، الرقم الضريبي، البريد، الهاتف..."

Table Layout:
  Columns: [أفاتار] | الاسم | الأدوار | البريد | الهاتف | صافي الرصيد | الإجراءات
  
  Sample rows:
  ┌──────────────────────────────────────────────────────────────────────┐
  │ [STC logo] شركة الاتصالات السعودية  [عميل 🔵][مورد 🟢]  +205,000 │
  │ [ط.م]     طارق ملفي                [موظف 🟣][مساهم 🔵]  —        │
  │ [ع.س]     عبدالله السعيد           [فري لانسر 🟡]       -3,500   │
  │ [أ.م]     أحمد محمد                (جهة تواصل في STC)   —        │
  └──────────────────────────────────────────────────────────────────────┘

Avatar Rules:
  - Organizations: logo from LinkedIn/Clearbit or manual upload
  - Persons with photo: circular photo
  - Persons without photo: initials (2 chars) with deterministic color from name hash
  - Role badges always visible next to name (colored pills)

"إضافة جهة +" Flow:
  Step 1: هل هي منظمة أم فرد؟ [منظمة 🏢] [فرد 👤]
  Step 2 (Organization):
    - اسم المنظمة → AUTO-SEARCH LinkedIn/Clearbit for logo + data
    - "هل تقصد: شركة الاتصالات السعودية (STC)؟" with logo preview
    - Smart dedup: checks tax_number, commercial_reg, name similarity
    - If match found: "هذه المنظمة مسجلة. هل تريد إضافة دور جديد?"
  Step 2 (Person):
    - الاسم + البريد/الهاتف → dedup check
    - Auto-generate avatar from initials
  Step 3: اختر الأدوار (multi-select checkboxes):
    ☐ عميل  ☐ مورد  ☐ موظف  ☐ فري لانسر  ☐ مساهم  ☐ مستثمر
  Step 4: بيانات كل دور (conditional sections appear based on selection)

Click on any contact → Party Portfolio Page (/contacts/:id)
```

### Screen 6b: Party Portfolio Page — ملف الجهة الشامل
```
Route: /contacts/:partyId

Header:
  ┌────┐
  │Logo│ شركة الاتصالات السعودية (STC)
  │ STC│ [عميل 🔵] [مورد 🟢]  [🔗 LinkedIn] [🌐 stc.com.sa]
  └────┘ الرقم الضريبي: 300000000000003 | الرياض

KPI Cards (role-aware, showing both sides):
  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
  │ كعميل لنا    │ │ كمورد لنا    │ │ الرصيد الصافي│
  │ 250,000 ر.س  │ │ 45,000 ر.س   │ │ +205,000 ر.س │
  │ 12 فاتورة    │ │ 6 فواتير     │ │ لصالحنا ✅   │
  └──────────────┘ └──────────────┘ └──────────────┘

Tabs: [نظرة عامة] [العمليات] [الأشخاص] [المستندات] [البوابة] [السجل]

Tab 1 — نظرة عامة: Company info + addresses + contact methods
Tab 2 — العمليات: Filterable timeline (invoices, payments, all transactions)
         Each transaction shows which contact person was involved
Tab 3 — الأشخاص: Contact persons linked to this org
         Each with avatar, role, department, last activity
         [+ ربط شخص بهذه المنظمة]
Tab 4 — المستندات: Contracts, attachments, uploaded files
Tab 5 — البوابة: Portal access management (who has access, what level)
         [+ دعوة شخص للبوابة]
         Access levels: شخصي | قسم | منظمة كاملة | مدقق
Tab 6 — السجل: Full activity timeline (chronological)
```
