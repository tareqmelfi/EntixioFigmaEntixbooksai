...
export function AdminJoinPage() {
  ...
  return (
    <div className="min-h-dvh bg-canvas flex items-center justify-center p-6" dir={language === "ar" ? "rtl" : "ltr"}>
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-[0_8px_24px_rgba(11,27,73,0.08)]">
        <div className="flex items-center gap-2 mb-4"><span className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-[#1A1E48] text-primary-foreground"><ShieldCheck className="h-5 w-5" /></span><div><div dir="ltr" lang="en" className="font-english text-foreground" style={{ fontWeight: 800 }}><EntixWordmark size={16} /> · Admin</div><div className="text-xs text-muted-foreground">{t("دعوة للانضمام إلى فريق المنصة", "Invitation to the platform team")}</div></div></div>
        {err ? <div className="flex items-start gap-2 rounded-lg border border-danger-border bg-danger-subtle p-3 text-sm text-danger"><AlertTriangle className="h-4 w-4 mt-0.5" />{err}</div>
        : !info ? <Loader2 className="h-6 w-6 animate-spin text-primary mx-auto my-8" />
        : done ? <div className="flex items-center gap-2 rounded-lg border border-success-border bg-success-subtle p-3 text-sm text-success"><CheckCircle2 className="h-4 w-4" />{t("تم القبول — جارٍ فتح لوحة الإدارة…", "Accepted — opening the console…")}</div>
        ...
        : info.expired ? <p className="text-sm text-warning">{t("انتهت صلاحية الدعوة — اطلب دعوة جديدة.", "This invitation expired — ask for a new one.")}</p>
        : (
          <div className="space-y-3 text-sm">
            ...
            {auth.loading ? <Loader2 className="h-5 w-5 animate-spin text-primary" /> : !auth.isAuthenticated ? (
              <div className="space-y-2">
                ...
                <Link to={`/login?next=${encodeURIComponent(`/admin/join/${token}`)}`} className="inline-flex rounded-lg bg-primary px-4 py-2 text-primary-foreground" style={{ fontWeight: 600 }}>{t("تسجيل الدخول", "Sign in")}</Link>
              ...
              <p className="text-warning">{t("أنت مسجّل بحساب مختلف", "You are signed in with a different account")} (<span className="font-english" dir="ltr">{auth.user?.email}</span>). {t("سجّل الخروج وادخل بالبريد المدعو.", "Sign out and sign in with the invited email.")}</p>
            ) : (
              <button onClick={() => void accept()} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-primary-foreground" style={{ fontWeight: 600 }}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}{t("قبول الدعوة", "Accept invitation")}</button>
            )}
          </div>
        )}
      </div>
