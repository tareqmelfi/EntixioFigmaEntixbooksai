...
const statusBadge = (s: string) => {
  const m: Record<string, string> = {
    ...
    "مسودة": "bg-muted/50 text-muted-foreground",
  };
  return m[s] || "";
};
    ...
    <div className="min-h-screen bg-muted" dir="rtl">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <div className="text-end">
                <div className="text-xs text-foreground" style={{ fontWeight: 500 }}>أحمد محمد</div>
                <div className="text-[10px] text-muted-foreground font-english">ahmed@stc.com.sa</div>
              </div>
            </div>
      ...
      <div className="bg-white border-b border-border px-6 py-2">
        <div className="max-w-5xl mx-auto flex items-center gap-2">
          <span className="text-xs text-muted-foreground">تصفح كـ:</span>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6 space-y-6">
        {activeTab === "home" && !selectedInvoice && (
          <>
            <div>
              <h2 className="text-foreground" style={{ fontSize: "1.25rem", fontWeight: 700 }}>مرحباً أحمد 👋</h2>
              <p className="text-sm text-muted-foreground mt-0.5">بوابة: شركة ENTIX.IO العالمية</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              <Card className="border-border">
                <CardContent className="pt-4 pb-3 px-4 text-center">
                  <div className="text-foreground font-english" style={{ fontSize: "1.5rem", fontWeight: 700 }}>{totalInvoices}</div>
              ...
              <Card className="border-border relative overflow-hidden">
                ...
                <CardContent className="pt-4 pb-3 px-4 text-center">
                  <div dir="ltr" className="flex items-baseline justify-center gap-1">
                    ...
                    <span className="text-xs text-muted-foreground font-english">{CUR}</span>
                  </div>
                  <p className="text-xs text-muted-foreground mt-0.5">مدفوع ✅</p>
                </CardContent>
              </Card>
              <Card className="border-border relative overflow-hidden">
            ...
            <Card className="border-border">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-foreground" style={{ fontSize: "1rem" }}>آخر الفواتير</CardTitle>
                  <button onClick={() => setActiveTab("invoices")} className="text-xs text-primary hover:underline" style={{ fontWeight: 500 }}>عرض الكل →</button>
                </div>
              </CardHeader>
              <CardContent>
                <div className="space-y-0">
                  {portalInvoices.slice(0, 3).map((inv) => (
                    <button key={inv.id} onClick={() => setSelectedInvoice(inv.id)} className="w-full flex items-center justify-between py-3 border-b border-border/50 last:border-0 hover:bg-muted transition-colors text-start px-2 rounded-md">
                      <div className="flex items-center gap-3">
                        <span className="font-english text-sm text-primary" style={{ fontWeight: 600 }}>{inv.id}</span>
                        <span className="text-[#D1D5DB]">|</span>
                        <span className="font-english text-sm text-muted-foreground">{inv.date}</span>
                        <span className="text-[#D1D5DB]">|</span>
                        <span dir="ltr" className="font-english text-sm text-foreground" style={{ fontWeight: 600 }}>{CUR} {inv.amount.toLocaleString()}</span>
                      </div>
            ...
            <Card className="border-border">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-foreground" style={{ fontSize: "1rem" }}>المستندات المشتركة</CardTitle>
                  <button onClick={() => setActiveTab("documents")} className="text-xs text-primary hover:underline" style={{ fontWeight: 500 }}>عرض الكل</button>
                </div>
              </CardHeader>
              <CardContent>
            ...
            <button className="flex items-center gap-2 rounded-lg border border-[#0B1B49] px-4 py-2.5 text-sm text-foreground hover:bg-[#ECEEF5] transition-colors" style={{ fontWeight: 500 }}>
            ...
            <Card className="border-border">
              <CardContent className="pt-6 pb-5">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-foreground font-english" style={{ fontSize: "1.25rem", fontWeight: 700 }}>{viewingInvoice.id}</h2>
                ...
                <div className="grid grid-cols-2 gap-6 mb-6 p-4 rounded-lg bg-muted">
                  <div>
                    <p className="text-xs text-muted-foreground/60" style={{ fontWeight: 500 }}>من</p>
                    <p className="text-sm text-foreground mt-0.5" style={{ fontWeight: 600 }}>شركة ENTIX.IO العالمية</p>
                    <p className="text-xs text-muted-foreground">الرياض، المملكة العربية السعودية</p>
                    <p className="text-xs text-muted-foreground font-english mt-0.5">VAT: 300000000000001</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground/60" style={{ fontWeight: 500 }}>إلى</p>
                    <p className="text-sm text-foreground mt-0.5" style={{ fontWeight: 600 }}>شركة الاتصالات السعودية</p>
                    <p className="text-xs text-muted-foreground">الرياض، طريق الملك فهد</p>
                    <p className="text-xs text-muted-foreground font-english mt-0.5">VAT: 300000000000003</p>
                  </div>
                </div>
                  ...
                  <thead>
                    <tr className="border-b border-border">
                      {["الوصف", "الكمية", "السعر", "الضريبة", "الإجمالي"].map((h) => (
                        <th key={h} className="pb-2 pe-3 text-start text-xs text-muted-foreground" style={{ fontWeight: 600 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-b border-border/50">
                      <td className="py-3 pe-3 text-sm text-foreground/80">{viewingInvoice.description}</td>
                      <td className="py-3 pe-3 text-sm font-english text-foreground/80">1</td>
                      <td className="py-3 pe-3 text-sm font-english text-foreground/80">{(viewingInvoice.amount / 1.15).toFixed(0)}</td>
                      <td className="py-3 pe-3 text-sm font-english text-foreground/80">15%</td>
                      <td className="py-3 pe-3 text-sm font-english text-foreground" style={{ fontWeight: 600 }}>{viewingInvoice.amount.toLocaleString()}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                <div className="flex justify-end">
                  <div className="w-64 space-y-1.5">
                    <div className="flex justify-between text-sm text-muted-foreground"><span>المجموع</span><span dir="ltr" className="font-english">{CUR} {(viewingInvoice.amount / 1.15).toFixed(0)}</span></div>
                    <div className="flex justify-between text-sm text-muted-foreground"><span>الضريبة (15%)</span><span dir="ltr" className="font-english">{CUR} {(viewingInvoice.amount - viewingInvoice.amount / 1.15).toFixed(0)}</span></div>
                    <div className="flex justify-between pt-2 border-t border-border"><span className="text-sm text-foreground" style={{ fontWeight: 700 }}>الإجمالي</span><span dir="ltr" className="font-english text-foreground" style={{ fontWeight: 700 }}>{CUR} {viewingInvoice.amount.toLocaleString()}</span></div>
                  </div>
                </div>
                ...
                <tbody>
                  {portalInvoices.map(inv => (
                    <tr key={inv.id} className="border-b border-border/50 last:border-0 hover:bg-muted transition-colors cursor-pointer" onClick={() => setSelectedInvoice(inv.id)}>
                      <td className="py-3 pe-3 text-sm font-english text-primary" style={{ fontWeight: 600 }}>{inv.id}</td>
                      <td className="py-3 pe-3 text-sm font-english text-muted-foreground">{inv.date}</td>
                      <td className="py-3 pe-3 text-sm text-foreground/80">{inv.description}</td>
                      <td className="py-3 pe-3"><span dir="ltr" className="font-english text-sm text-foreground" style={{ fontWeight: 600 }}>{CUR} {inv.amount.toLocaleString()}</span></td>
        ...
        {activeTab === "statement" && (
          <Card className="border-border">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-foreground" style={{ fontSize: "1rem" }}>كشف الحساب</CardTitle>
                <button className="flex items-center gap-1.5 rounded-lg border border-[#0B1B49] px-3 py-1.5 text-xs text-foreground hover:bg-[#ECEEF5] transition-colors" style={{ fontWeight: 500 }}>
            ...
            <CardContent>
                ...
                <tbody>
                  {portalStatements.map((s, i) => (
                    <tr key={i} className="border-b border-border/50 last:border-0 hover:bg-muted">
                      <td className="py-3 pe-3 text-sm font-english text-muted-foreground">{s.date}</td>
              ...
              <div className="mt-4 pt-3 border-t border-border text-end">
                ...
                <span dir="ltr" className="font-english text-foreground" style={{ fontWeight: 700 }}>{CUR} {portalStatements[0].balance.toLocaleString()}</span>
              </div>
            </CardContent>
          </Card>
        )}
                ...
                <div key={doc.id} className="flex items-center justify-between py-3 border-b border-border/50 last:border-0 hover:bg-muted transition-colors px-2 rounded-md">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground/60 font-english">{doc.date}</div>
                    </div>
                  </div>
