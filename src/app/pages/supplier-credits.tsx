      ...
      <>
        <FullPageForm
          ...
          footer={
            <div className="flex items-center justify-end gap-2">
              <Button type="button" variant="outline" onClick={() => setCreateOpen(false)} className="border-border">إلغاء</Button>
        ...
        >
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            <div className="space-y-2">
              <Label>ملاحظات</Label>
              <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={3} placeholder="تفاصيل الإرجاع أو الخصم..." className="w-full rounded-md border border-border px-3 py-2 text-sm" />
            </div>
          </div>
        </FullPageForm>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-foreground" style={{ fontSize: "1.75rem", fontWeight: 700 }}>إشعارات الموردين</h1>
          <p className="text-muted-foreground mt-1">مرتجعات وخصومات الموردين المرتبطة بفواتير المشتريات</p>
        </div>
        <Button className="bg-primary hover:bg-primary/90" onClick={openCreate}><Plus className="me-2 h-4 w-4" />إشعار مورد جديد</Button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-border"><CardContent className="p-5"><div className="text-muted-foreground text-sm mb-1">إجمالي الإشعارات</div><div className="font-english text-foreground text-xl font-semibold">{items.length}</div></CardContent></Card>
        <Card className="border-border"><CardContent className="p-5"><div className="text-muted-foreground text-sm mb-1">إجمالي القيمة</div><div className="font-english text-amber-600 text-xl font-semibold">{total.toLocaleString()}</div></CardContent></Card>
        <Card className="border-border"><CardContent className="p-5"><div className="text-muted-foreground text-sm mb-1">مطبَّقة</div><div className="font-english text-green-600 text-xl font-semibold">{items.filter((item) => item.status === "APPLIED").length}</div></CardContent></Card>
      </div>

      <Card className="border-border">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-foreground">قائمة إشعارات الموردين</CardTitle>
        ...
        <CardContent>
          ...
          filtered.length === 0 ? (
            <div className="py-12 text-center"><ScrollText className="h-12 w-12 mx-auto text-muted-foreground/60 mb-3" /><p className="text-sm text-muted-foreground">لا توجد إشعارات موردين</p></div>
          ) : (
              ...
              <tbody>
                {filtered.map((item) => (
                  <tr key={item.id} className="border-b border-border/50 hover:bg-primary/5">
                    ...
                    <td className="py-3 px-4 text-xs text-muted-foreground">{REASONS.find((reason) => reason.value === item.reason)?.label || item.reason}</td>
