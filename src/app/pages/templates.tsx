  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-foreground" style={{ fontSize: "1.75rem", fontWeight: 700 }}>القوالب</h1>
          <p className="text-muted-foreground mt-1">إدارة قوالب الفواتير والمستندات</p>
        </div>
        <Button className="bg-primary hover:bg-primary/90"><Plus className="me-2 h-4 w-4" />قالب جديد</Button>
      </div>

      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground/60" />
          <Input placeholder="بحث في القوالب..." className="ps-10 border-border" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
      </div>
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-foreground" style={{ fontWeight: 600 }}>{template.name}</div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs text-muted-foreground">{template.type}</span>
                        {template.isDefault && (
                          <span className="inline-flex rounded-full bg-[#ECEEF5] px-2 py-0.5 text-[10px] text-foreground" style={{ fontWeight: 600 }}>افتراضي</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] ${template.status === "نشط" ? "bg-[#ECEEF5] text-foreground" : "bg-[#FEF3C7] text-[#92400E]"}`} style={{ fontWeight: 600 }}>{template.status}</span>
                </div>
                <p className="text-sm text-muted-foreground mb-3">{template.description}</p>
                <div className="flex items-center justify-between pt-3 border-t border-border/50">
                  <span className="text-xs text-muted-foreground/60 font-english">{template.lastModified}</span>
                  <div className="flex gap-1">
                    <button className="rounded-md p-1.5 text-muted-foreground hover:bg-muted/50 transition-colors"><Eye className="h-3.5 w-3.5" /></button>
                    <button className="rounded-md p-1.5 text-muted-foreground hover:bg-muted/50 transition-colors"><Edit2 className="h-3.5 w-3.5" /></button>
                    <button className="rounded-md p-1.5 text-muted-foreground hover:bg-muted/50 transition-colors"><Copy className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
              </CardContent>
            </Card>
