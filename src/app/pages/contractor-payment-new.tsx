...
export function ContractorPaymentNew() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <Link to={`/app/contractors/${person.id}`} className="mb-1 inline-flex items-center gap-1.5 text-xs text-content-secondary hover:text-primary">
          <ArrowRight className="h-3.5 w-3.5 ltr:rotate-180" strokeWidth={1.75} /> {t("الموظفون والرواتب", "Employees & payroll")} · {t("العودة لصفحة المقاول", "Back to contractor")} · {person.name}
        </Link>
        <h1 className="text-[clamp(1.75rem,1.5rem+0.8vw,2.25rem)] font-bold leading-tight tracking-[-0.01em] text-foreground">{t("دفعة مباشرة للمقاول", "Direct contractor payment")}</h1>
        <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">{t("قيد واحد مباشر: مدين 67200 أتعاب مقاولين · دائن البنك — بدون فاتورة شراء ولا دورة ذمم", "One direct entry: Dr 67200 Subcontractor Fees · Cr bank — no bill, no AP cycle")}</p>
      </div>
