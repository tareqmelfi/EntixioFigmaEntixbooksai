...
const FEATURES_SA = [
  { icon: FileText, title: "فواتير احترافية", titleEn: "Professional invoices", desc: "إنشاء وإدارة الفواتير مع QR يحتوي بيانات الفاتورة الأساسية. حالة الربط خاصة بكل منشأة. راجع الإعدادات ← ZATCA للتحقق من بيئة الجهاز وصلاحية شهادته وحالة إرسال الفواتير. وجود شهادة لا يثبت قبول كل فاتورة، والتفعيل الإنتاجي يتطلب استكمال التحقق الخاص بالمنشأة.", descEn: "Create and manage invoices with a QR containing core invoice data. Onboarding status is specific to each organization. Check Settings → ZATCA for the device environment, certificate validity and invoice submission status. A certificate does not prove acceptance of every invoice; production activation requires organization-specific verification." },
...
export function Landing() {
  ...
  const faqs = [
    ...(isSA ? [{ q: t("كيف أربط منشأتي بمنصة فاتورة؟", "How do I connect my organization to Fatoora?"), a: <>{t("يمكننا مساعدتك في الربط، أو تستخرج رمز تهيئة الجهاز بنفسك من بوابة فاتورة. الرمز صالح ساعة ويختلف عن رمز دخول الجوال. اتبع ", "Our team can assist, or you can generate the device OTP in Fatoora yourself. It is valid for one hour and differs from your SMS login code. Follow the ")}<a className="text-primary underline" href="/guides/zatca-onboarding-ar.html">{t("الدليل المصور", "illustrated Arabic guide")}</a>{t("، أو ", ", or ")}<a className="text-primary underline" href="/guides/zatca-onboarding-ar.pdf" download>{t("حمّل ملف PDF", "download the Arabic PDF")}</a>.</> }] : []),
    isSA
      ? { q: t("ما حالة تكامل الفوترة الإلكترونية في السعودية؟", "What is the status of Saudi e-invoicing integration?"), a: t("حالة الربط خاصة بكل منشأة. راجع الإعدادات ← ZATCA للتحقق من بيئة الجهاز وصلاحية شهادته وحالة إرسال الفواتير. وجود شهادة لا يثبت قبول كل فاتورة، والتفعيل الإنتاجي يتطلب استكمال التحقق الخاص بالمنشأة.", "Onboarding status is specific to each organization. Check Settings → ZATCA for the device environment, certificate validity and invoice submission status. A certificate does not prove acceptance of every invoice; production activation requires organization-specific verification.") }
