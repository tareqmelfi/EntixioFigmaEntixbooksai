            ...
            {workspace.subscription.availability !== "available" ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 text-sm">
                  ...
                  <div className="rounded-lg border border-border p-3"><div className="text-xs text-muted-foreground">{t("الخطة", "Plan")}</div><div className="text-foreground" style={{ fontWeight: 700 }}>{workspace.subscription.data.plan?.name || "—"}</div><div className="text-[11px] text-muted-foreground font-english" dir="ltr">{workspace.subscription.data.plan ? `${workspace.subscription.data.plan.tier} · ${workspace.subscription.data.plan.interval} · ${(workspace.subscription.data.plan.price / 100).toLocaleString(displayLocale("en-US"), { maximumFractionDigits: 2 })} ${workspace.subscription.data.plan.currency.toUpperCase()}` : ""}</div></div>
