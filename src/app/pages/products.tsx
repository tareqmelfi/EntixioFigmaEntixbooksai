  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-foreground" style={{ fontSize: "1.75rem", fontWeight: 700 }}>المنتجات والخدمات</h1><p className="text-muted-foreground mt-1">إدارة المنتجات والخدمات والأصناف المخزنية</p></div>
        <Button className="bg-primary hover:bg-primary/90" onClick={openCreate}><Plus className="me-2 h-4 w-4" />صنف جديد</Button>
      </div>
      <Card className="border-border"><CardHeader><CardTitle>القائمة · {items.length}</CardTitle></CardHeader><CardContent>
        {loading ? <div className="py-8 text-center"><Loader2 className="h-6 w-6 animate-spin mx-auto text-primary" /></div> :
         items.length === 0 ? <div className="py-12 text-center"><Package className="h-12 w-12 mx-auto text-muted-foreground/60 mb-3" /><p className="text-sm text-muted-foreground">لا توجد منتجات</p></div> :
            ...
            {items.map(p => {
              ...
              <td className="py-3 px-4">
                <div className="flex items-center gap-1">
                  <button onClick={() => openEdit(p)} className="rounded-md p-1.5 text-primary hover:bg-blue-50" title="تعديل"><Edit2 className="h-4 w-4" /></button>
      ...
      <SidePanel open={open} onClose={() => setOpen(false)}>
        <div className="mb-3"><h2 className="text-foreground text-lg font-semibold">{editingId ? "تعديل صنف" : "صنف جديد"}</h2></div>
          <form onSubmit={handleSubmit} className="space-y-4 py-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <label htmlFor="product-image-upload" className="cursor-pointer rounded-md border border-[#1276E3] px-3 py-2 text-xs text-primary hover:bg-blue-50">اختيار صورة</label>
                    {form.imageUrl && <button type="button" onClick={() => setForm({ ...form, imageUrl: "" })} className="rounded-md border border-border px-3 py-2 text-xs text-muted-foreground hover:bg-muted"><X className="inline h-3.5 w-3.5 me-1" />إزالة</button>}
                  </div>
            ...
            <div className="flex items-center justify-end gap-2 mt-4 pt-3 border-t border-border"><Button type="button" variant="outline" onClick={() => setOpen(false)}>إلغاء</Button><Button type="submit" disabled={busy} className="bg-primary hover:bg-primary/90">{busy ? "..." : editingId ? "تحديث" : "حفظ"}</Button></div>
          </form>
        </SidePanel>
