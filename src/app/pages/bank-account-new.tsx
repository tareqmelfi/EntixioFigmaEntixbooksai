...
export function BankAccountNew() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <Link to="/app/bank-accounts" className="mb-1 inline-flex items-center gap-1.5 text-xs text-content-secondary hover:text-primary">
          <ArrowRight className="h-3.5 w-3.5 ltr:rotate-180" strokeWidth={1.75} /> {t("البنوك والنقد", "Banks & cash")} · {t("العودة للحسابات البنكية", "Back to Bank accounts")}
        </Link>
        <h1 className="text-[clamp(1.75rem,1.5rem+0.8vw,2.25rem)] font-bold leading-tight tracking-[-0.01em] text-foreground">{t("حساب بنكي جديد", "New bank account")}</h1>
        <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground flex items-center gap-1.5">
