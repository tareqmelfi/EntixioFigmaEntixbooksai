...
export function StockMovementNew() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <Link to="/app/stock-movements" className="mb-1 inline-flex items-center gap-1.5 text-xs text-content-secondary hover:text-primary">
          <ArrowRight className="h-3.5 w-3.5 ltr:rotate-180" strokeWidth={1.75} /> {t("المنتجات والمخزون", "Products & inventory")} · {t("العودة لسجل الحركات", "Back to Movement Log")}
        </Link>
        <h1 className="text-[clamp(1.75rem,1.5rem+0.8vw,2.25rem)] font-bold leading-tight tracking-[-0.01em] text-foreground">{t("حركة مخزون", "Stock movement")}</h1>
        <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">{t("سجل استلام، صرف، أو تحويل بين المستودعات.", "Record a receipt, issue, or transfer between warehouses.")}</p>
      </div>
