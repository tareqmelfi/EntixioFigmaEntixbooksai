...
function SectionHeading({ eyebrow, title, description, inverse = false }: { eyebrow: string; title: string; description?: string; inverse?: boolean }) {
  return <div className="mx-auto mb-10 max-w-3xl text-center">
    <span data-heading-eyebrow className={`mb-3 inline-block rounded-full px-4 py-1.5 text-sm font-semibold ${inverse ? "bg-card/15 text-primary-foreground" : "bg-primary/5 text-primary"}`}>{eyebrow}</span>
    <h2 data-heading-title className={`text-2xl sm:text-3xl font-bold ${inverse ? "text-primary-foreground" : "text-foreground"}`}>{title}</h2>
    {description && <p data-heading-description className={`mt-4 leading-8 ${inverse ? "text-primary-foreground" : "text-muted-foreground"}`}>{description}</p>}
  </div>;
}
...
function SolutionShell({ data, children }: { data: SolutionData; children?: React.ReactNode }) {
  ...
  return <div className="min-h-screen overflow-x-clip bg-card" dir={language === "ar" ? "rtl" : "ltr"}>
    <SharedNavbar />
    <main data-page={data.pageMarker} dir={language === "ar" ? "rtl" : "ltr"}>
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-[1.2fr_.8fr]">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link to="/register" className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3.5 font-semibold text-primary-foreground">{t("ابدأ مجاناً", "Start free")}<ArrowRight className="h-4 w-4" /></Link>
          ...
          <div className="rounded-3xl bg-gradient-to-br from-foreground to-primary p-7 text-primary-foreground shadow-2xl">
            <Icon className="h-9 w-9 text-primary-foreground/80" />
            <h2 className="mt-5 text-xl font-bold">{t("مبني للعمل الحقيقي", "Built for real work")}</h2>
            <ul className="mt-5 space-y-4">{data.proof.map((item) => <li key={item.en} className="flex gap-3 text-primary-foreground/80"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-secondary" /><span>{tx(language, item)}</span></li>)}</ul>
          </div>
        </div>
      </section>
      <section data-section="role-proof" className="bg-muted/40 px-4 py-16 sm:px-6">
        ...
        <div className="mx-auto grid max-w-6xl gap-5 md:grid-cols-3">{data.proof.map((item) => <div key={item.en} className="rounded-2xl border border-border bg-card p-6"><CheckCircle2 className="h-5 w-5 text-primary" /><p className="mt-4 font-semibold leading-7 text-foreground">{tx(language, item)}</p></div>)}</div>
      </section>
      ...
      <section data-section="market-compliance" className="bg-foreground px-4 py-16 text-primary-foreground sm:px-6"><div className="mx-auto max-w-4xl text-center"><ShieldCheck className="mx-auto h-8 w-8 text-secondary" /><h2 className="mt-4 text-2xl sm:text-3xl font-bold">{isSA ? t("السوق السعودي", "Saudi market") : t("السوق الأمريكي", "US market")}</h2><p className="mt-5 leading-8 text-primary-foreground/75">{market}</p></div></section>
      ...
      <section data-section="onboarding" className="bg-muted/40 px-4 py-16 sm:px-6"><SectionHeading eyebrow={t("البدء", "Onboarding")} title={t("ابدأ بخطوات عملية", "Start with practical steps")} /><ol className="mx-auto grid max-w-5xl gap-5 md:grid-cols-3">{data.onboarding.map((item, index) => <li key={item.en} className="rounded-2xl bg-card p-6"><span className="text-sm font-bold text-primary">0{index + 1}</span><p className="mt-3 leading-7 text-foreground">{tx(language, item)}</p></li>)}</ol></section>
      {children}
      <section data-section="final-cta" className="px-4 py-16 pb-24 sm:px-6"><div className="mx-auto max-w-4xl rounded-3xl bg-gradient-to-br from-foreground to-primary p-8 text-center text-primary-foreground sm:p-12"><h2 className="text-2xl sm:text-3xl font-bold">{t("اختبر سير العمل على بياناتك", "Test the workflow with your own data")}</h2><p className="mx-auto mt-4 max-w-2xl leading-7 text-primary-foreground/75">{t("ابدأ بمساحة عمل، وتحقق من الملاءمة والامتثال والتكاملات المطلوبة قبل الاعتماد الكامل.", "Start a workspace and validate fit, compliance, and required integrations before full adoption.")}</p><Link to="/register" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-card px-7 py-3.5 font-bold text-foreground">{t("سجّل حساباً", "Sign up")}<ArrowRight className="h-4 w-4" /></Link></div></section>
    </main>
    <SharedFooter />
  </div>;
}
...
function AccountantsPage() {
  ...
  const faq = isSA ? [
    ...
    copy("يوفر مسارات لضريبة القيمة المضافة، بينما تكامل ZATCA للمرحلة الثانية قيد التحقق الفني والتنظيمي وغير مفعّل للاعتماد الإنتاجي.", "It provides VAT workflows, while ZATCA Phase 2 integration is under technical and regulatory validation and is not enabled for production reliance."),
  ] : [
    copy("هل تغني المنصة عن مسؤولية المحاسب؟", "Does the platform replace accountant responsibility?"),
  ...
  return <div className="min-h-screen overflow-x-clip bg-card" dir={language === "ar" ? "rtl" : "ltr"}>
    ...
    <main data-page="solutions-accountants" dir={language === "ar" ? "rtl" : "ltr"}>
      ...
      <section data-section="market-compliance" className="bg-foreground px-4 py-16 text-primary-foreground sm:px-6"><div className="mx-auto max-w-5xl"><SectionHeading inverse eyebrow={t("السوق والامتثال", "Market and compliance")} title={isSA ? t("السعودية: ضريبة القيمة المضافة وZATCA Phase 2 قيد التحقق", "Saudi Arabia: VAT and ZATCA Phase 2 under validation") : t("الولايات المتحدة: ضريبة المبيعات و1099 وربط البنوك", "United States: sales tax, 1099, and bank feeds")} description={isSA ? t("راجع ملف العميل الضريبي وإعدادات الفاتورة قبل الاعتماد. تكامل ZATCA للمرحلة الثانية قيد التحقق الفني والتنظيمي وغير مفعّل للاعتماد الإنتاجي، ولا تستبدل المنصة الإقرار أو الاستشارة المهنية.", "Review the client's tax profile and invoice settings before reliance. ZATCA Phase 2 integration is under technical and regulatory validation and is not enabled for production reliance; the platform does not replace filing or professional advice.") : t("راجع قواعد ضريبة المبيعات حسب الولاية، وتتبع موردي 1099، وسوِّ الحركات البنكية. تتوفر bank feeds بحسب الحساب والتكامل، ولا تتضمن الصفحة ادعاء تقديم الإقرارات تلقائياً.", "Review state-specific sales tax rules, track 1099 vendors, and reconcile transactions. Bank feeds depend on the account and available integration; this page does not claim automatic tax filing.")} /></div></section>
      ...
      <section data-section="faq" className="px-4 py-16 sm:px-6"><SectionHeading eyebrow={t("الأسئلة الشائعة", "FAQ")} title={t("ما يجب تأكيده قبل الاعتماد", "What to confirm before adoption")} /><div className="mx-auto max-w-4xl space-y-4">{[0, 2].map((index) => <details key={faq[index].en} className="rounded-2xl border border-border bg-card p-6" open={index === 0}><summary className="cursor-pointer font-bold text-foreground">{tx(language, faq[index])}</summary><p className="mt-4 leading-8 text-muted-foreground">{tx(language, faq[index + 1])}</p></details>)}</div></section>
      <section data-section="final-cta" className="px-4 py-16 pb-24 sm:px-6"><div className="mx-auto max-w-4xl rounded-3xl bg-gradient-to-br from-foreground to-primary p-8 text-center text-primary-foreground sm:p-12"><ShieldCheck className="mx-auto h-8 w-8 text-primary-foreground/80" /><h2 className="mt-4 text-2xl sm:text-3xl font-bold">{t("جرّب سير عمل عميل واحد أولاً", "Pilot one client workflow first")}</h2><p className="mx-auto mt-4 max-w-2xl leading-7 text-primary-foreground/75">{t("تحقق من البيانات والصلاحيات والامتثال والتكاملات المطلوبة، ثم وسّع الاستخدام بناءً على نتيجة المراجعة.", "Validate data, permissions, compliance, and required integrations, then expand based on the review outcome.")}</p><div className="mt-7 flex flex-col items-center justify-center gap-3 sm:flex-row"><Link to="/register" className="inline-flex items-center gap-2 rounded-xl bg-card px-7 py-3.5 font-bold text-foreground">{t("سجّل حساباً", "Sign up")}<ArrowRight className="h-4 w-4" /></Link><a href="#accountant-workflow" className="rounded-xl border border-card/30 px-7 py-3.5 font-semibold text-primary-foreground">{t("سير عمل المحاسب", "Accountant workflow")}</a></div></div></section>
    </main>
    <SharedFooter />
  </div>;
}
