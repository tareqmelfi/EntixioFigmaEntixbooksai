...
export function WarehouseNew() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <Link to="/app/warehouses" className="mb-1 inline-flex items-center gap-1.5 text-xs text-content-secondary hover:text-primary">
          <ArrowRight className="h-3.5 w-3.5 ltr:rotate-180" strokeWidth={1.75} /> {t("المنتجات والمخزون", "Products & inventory")} · {t("العودة للمستودعات", "Back to Warehouses")}
        </Link>
        <h1 className="text-[clamp(1.75rem,1.5rem+0.8vw,2.25rem)] font-bold leading-tight tracking-[-0.01em] text-foreground">{t("مستودع جديد", "New warehouse")}</h1>
        <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground flex items-center gap-1.5"><Warehouse className="h-4 w-4" />{t("أضف مستودع أو فرع تخزين فعلي.", "Add a physical warehouse or storage branch.")}</p>
      </div>
