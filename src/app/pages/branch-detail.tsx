...
export function BranchDetail() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <Link to="/app/branches" className="mb-1 inline-flex items-center gap-1.5 text-xs text-content-secondary hover:text-primary">
          <ArrowRight className="h-3.5 w-3.5 ltr:rotate-180" strokeWidth={1.75} /> {t("المحاسبة", "Accounting")} · {t("العودة للفروع", "Back to Branches")}
        </Link>
        ...
        {isNew && <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground flex items-center gap-1.5"><GitBranch className="h-4 w-4" />{t("أضف فرعاً جديداً للشركة", "Add a new company branch")}</p>}
      </div>
