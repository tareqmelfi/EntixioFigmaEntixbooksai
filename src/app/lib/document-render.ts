...
export const DEFAULT_BRAND_COLOR = "#1276E3";
export const DEFAULT_COVER_COLOR = "#0B1B49";
...
.edoc{--brand:${brand};--dark:${dark};--ink:#111827;--muted:#5B6472;--soft:#F4F6F9;--rule:#DCE1E8;--paper:#fff;
...
.edoc .sheet.dark{background:var(--dark);color:#fff;--ink:#fff;--muted:rgba(255,255,255,.72);--rule:rgba(255,255,255,.18)}
...
.edoc .hdr-logo{display:flex;align-items:center;height:14mm;max-width:70mm}
.edoc .hdr-logo img{max-height:12mm;max-width:64mm;object-fit:contain;display:block}
.edoc .dark .hdr-logo img{background:#fff;border-radius:6px;padding:2px 6px}
.edoc .hdr-word{font-weight:800;font-size:12pt;letter-spacing:-.01em;color:var(--ink);white-space:nowrap;max-width:70mm;overflow:hidden;text-overflow:ellipsis;direction:${lang === "ar" ? "rtl" : "ltr"}}
...
/* cover */
.edoc .cover-body{display:flex;flex-direction:column;justify-content:center;height:100%;padding:0 0 6mm}
.edoc .eyebrow{font-family:var(--font-mono);font-size:7.5pt;letter-spacing:.14em;color:var(--brand);text-transform:uppercase}
.edoc .cover-title{font-size:30pt;font-weight:800;line-height:1.2;margin:4mm 0 6mm;letter-spacing:-.01em}
.edoc .cover-title .accent{color:var(--brand)}
.edoc .cover-rule{height:.5pt;background:var(--rule);margin:0 0 6mm}
.edoc .cover-intro{font-size:11.5pt;line-height:1.9;max-width:150mm;color:var(--ink)}
.edoc .cover-intro strong{font-weight:700}
.edoc .cover-meta{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8mm;margin-top:14mm;padding-top:8mm;border-top:.5pt solid var(--rule)}
...
.edoc .party .n2{font-family:var(--font-latin);font-size:8.5pt;color:var(--muted);direction:ltr;text-align:start}
...
.edoc .note{display:grid;grid-template-columns:6mm 1fr;gap:3mm;background:#EEF5FF;border:.5pt solid #BFD6F6;border-radius:2.5mm;padding:4mm 5mm;font-size:8.5pt;line-height:1.7;margin:0 0 6mm;break-inside:avoid}
...
.edoc .sig-cards{display:grid;grid-template-columns:1fr 1fr;gap:5mm;margin:0 0 6mm}
.edoc .sig .k{font-size:8pt;color:var(--muted);margin-bottom:1.5mm}
.edoc .sig .n{font-size:12pt;font-weight:800}
.edoc .sig .c{font-family:var(--font-mono);font-size:8pt;color:var(--muted);direction:ltr;text-align:start;margin-top:1mm}
.edoc .sig .o{font-size:8pt;color:var(--muted);margin-top:1mm}
.edoc .stamp-box{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:24mm}
.edoc .stamp-box img{max-height:28mm;max-width:50mm;object-fit:contain;opacity:.92;mix-blend-mode:multiply}
.edoc .stamp-box .ph{width:36mm;height:18mm;border:1pt dashed var(--rule);border-radius:2mm}
...
export function renderDocument(input: RenderInput): RenderOutput {
  ...
  const brand = safeColor(tpl.brandColor, "#1276E3");
  const dark = safeColor(tpl.coverColor, "#0B1B49");
  const coverStyle: CoverStyle = tpl.coverStyle === "LIGHT" ? "LIGHT" : tpl.coverStyle === "NONE" ? "NONE" : "DARK";
  ...
  const logo = tpl.showLogo === false ? "" : safeUrl(org.logoUrl);
  ...
  <div class="hdr-logo">${logo ? `<img src="${esc(logo)}" alt="${esc(orgName)}">` : `<span class="hdr-word">${esc(orgName)}</span>`}</div>
  ...
  <div class="cover-meta">
    ...
    <div><div class="k">${t("الإجمالي شامل الضريبة", "Total incl. tax")}</div>
...
<div class="meta-strip">
  ...
  const itemsBlock = (): Block => {
    const cols = `<colgroup><col style="width:30mm"><col><col style="width:16mm"><col style="width:26mm"><col style="width:28mm"></colgroup>`;
    const head = `<thead><tr><th>${t("الرمز", "Code")}</th><th>${t("البند", "Item")}</th><th class="n">${t("الكمية", "Qty")}</th><th class="n">${t("السعر", "Price")} (${esc(cur)})</th><th class="n">${t("المبلغ", "Amount")} (${esc(cur)})</th></tr></thead>`;
    ...
    const row = (l: LineSpec, i: number) => {
      ...
      const h = 9 + textHeight(headTxt, 80, 5, 1.9) + (rest ? textHeight(rest, 80, 4.4, 1.5) : 0);
      const code = l.code || (l.unit ? l.unit : "");
      return { h, html: `<tr><td><span class="code">${esc(code || String(i + 1).padStart(2, "0"))}</span></td><td><div class="head">${bdi(headTxt)}</div>${rest ? `<div class="rest">${bdi(rest)}</div>` : ""}</td><td class="n">${num(qty(l.quantity))}</td><td class="n">${num(money(l.unitPrice))}</td><td class="n">${num(money(l.subtotal))}</td></tr>` };
    };
  ...
  const companyBlock = (): Block => {
    ...
    const ids = [org.crNumber ? `${t("س.ت", "CR")} ${org.crNumber}` : "", org.vatNumber ? `${t("الرقم الضريبي", "VAT no.")} ${org.vatNumber}` : ""].filter(Boolean);
    if (ids.length) parts.push(`(${ids.map(esc).join(" · ")})`);
  ...
  const signatoryBlock = (): Block | null => {
    ...
    const sig = `<div class="card sig"><div class="k">${t("ممثل الشركة", "Company representative")}</div><div class="n">${bdi(tpl.signatoryName || "—")}</div>${tpl.signatoryTitle ? `<div class="o">${bdi(tpl.signatoryTitle)}</div>` : ""}${(tpl.signatoryEmail || tpl.signatoryPhone) ? `<div class="c">${[tpl.signatoryEmail, tpl.signatoryPhone].filter(Boolean).map(esc).join(" · ")}</div>` : ""}<div class="o">${bdi(ar ? org.name : (org.legalName || org.nameEn || org.name))}</div></div>`;
    const st = `<div class="card"><div class="k" style="font-size:8pt;color:var(--muted)">${t("ختم الشركة", "Company stamp")}</div><div class="stamp-box">${stamp ? `<img src="${esc(stamp)}" alt="">` : `<div class="ph"></div>`}</div></div>`;
    return { kind: "html", h: 38, html: `<div class="sig-cards">${sig}${st}</div>` };
  };
  ...
  const bodyHtml = sheets.map((s, i) => `<section class="sheet ${s.cls}" data-page="${i + 1}">${header()}${s.body}${footer(i + 1, total, !!s.cover)}</section>`).join("\n");
