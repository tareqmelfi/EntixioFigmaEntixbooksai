...
export async function renderDeviceProofDocument(proof: DeviceProof, t: (ar: string, en: string) => string) {
  if (!proof.certificate || !proof.deviceLinked) throw new Error("device_proof_unavailable");
  ...
  const text = (value: string, x: number, y: number, size: number, color = "#071d3e", bold = false, width = 2150) => {
  ...
  const field = (label: string, value: string, x: number, y: number) => {
    ...
    text(value, x, y + 76, 52, "#071d3e", true, 1010);
  };

  ctx.fillStyle = "#ffffff"; ctx.fillRect(0, 0, canvas.width, canvas.height);
  ctx.strokeStyle = "#071d3e"; ctx.lineWidth = 3; ctx.strokeRect(48, 48, 2384, 1658);
  ctx.strokeStyle = "#d7e2ed"; ctx.lineWidth = 1; ctx.strokeRect(66, 66, 2348, 1622);
  ctx.fillStyle = "#03b7d7"; ctx.fillRect(66, 66, 2348, 12);
  ...
  text(t("بوابة فاتورة", "Fatoora portal"), qrRight, 465, 31, "#071d3e", true, 340);
  text(t("تسجيل الدخول لمراجعة الجهاز", "Sign in to review the device"), qrRight, 510, 24, "#526077", false, 340);

  text("ENTIX.IO", 2325, 208, 68, "#071d3e", true, 1500);
  text(t("سجل ربط جهاز الفوترة الإلكترونية", "E-invoicing device onboarding record"), 2325, 350, 78, "#071d3e", true, 1710);
  ...
  text(proof.companyName, 2325, 760, 72, "#071d3e", true);
