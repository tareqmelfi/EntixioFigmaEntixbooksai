...
export const api = {
  ...
  admin: {
    ...
    orgDetail: (orgId: string, query?: { limit?: number; cursor?: string }) => request<{
      kind: 'organization_workspace';
      summary: { source: string; isAuthoritative: boolean; asOf: string; availability: 'available' | 'partial' | 'unavailable'; unavailableReason: string | null; data: { id: string; name: string; slug: string; country: string; baseCurrency: string; industry: string | null; createdAt: string; updatedAt: string } };
      metrics: { source: string; isAuthoritative: boolean; asOf: string; availability: 'available' | 'partial' | 'unavailable'; unavailableReason: string | null; data: { members: number; invoices: number; bills: number; expenses: number } };
      people: { source: string; isAuthoritative: boolean; asOf: string; availability: 'available' | 'partial' | 'unavailable'; unavailableReason: string | null; data: { items: Array<{ id: string; role: string; createdAt: string; user: { id: string; email: string; name: string | null; emailVerified: boolean } }>; nextCursor: string | null; hasMore: boolean } };
      subscription: { source: string; isAuthoritative: boolean; asOf: string; availability: 'available' | 'partial' | 'unavailable'; unavailableReason: string | null; data: { id: string; status: string; currentPeriodStart: string | null; currentPeriodEnd: string | null; cancelAtPeriodEnd: boolean; trialEndsAt: string | null; maskedStripeSubscriptionId: string | null; maskedStripeCustomerId: string | null; plan: { id: string; name: string; tier: string; interval: string; currency: string; price: number } | null } | null };
      support: { source: string; isAuthoritative: boolean; asOf: string; availability: 'available' | 'partial' | 'unavailable'; unavailableReason: string | null; data: { items: Array<{ id: string; title: string; createdAt: string; lastMessageAt: string; messageCount: number; user: { id: string; email: string; name: string | null } }>; nextCursor: string | null; hasMore: boolean } };
      activity: { source: string; isAuthoritative: boolean; asOf: string; availability: 'available' | 'partial' | 'unavailable'; unavailableReason: string | null; data: { items: Array<{ id: string; action: string; entityType: string; entityId: string | null; severity: string; occurredAt: string }>; nextCursor: string | null; hasMore: boolean } };
      referral: { source: string; isAuthoritative: boolean; asOf: string; availability: 'available' | 'partial' | 'unavailable'; unavailableReason: string | null; data: null };
      outstandingPlatformBilling: { source: string; isAuthoritative: boolean; asOf: string; availability: 'available' | 'partial' | 'unavailable'; unavailableReason: string | null; data: null };
    }>(`/api/admin/orgs/${orgId}`, { query, skipOrg: true }),
