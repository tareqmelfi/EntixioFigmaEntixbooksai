...
export function FullPageForm({ title, subtitle, onClose, children, footer, toolbar, disableEscape }: Props) {
  ...
  return (
    <div className="-m-6 sm:-m-8 min-h-[calc(100vh-4rem)] flex flex-col bg-[#F4FCFF]">
      {/* Header bar · sticky at top */}
      <div className="sticky top-0 z-10 bg-white border-b border-[#E5E7EB] shadow-sm">
        <div className="px-6 sm:px-8 py-4 flex items-center justify-between gap-4 flex-wrap">
          <div className="min-w-0">
            <h1 className="text-[#0B1B49] truncate" style={{ fontSize: "1.5rem", fontWeight: 700 }}>{title}</h1>
            {subtitle && <p className="text-[#6B7280] text-sm mt-0.5">{subtitle}</p>}
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-md p-2 text-[#6B7280] hover:bg-[#F3F4F6] hover:text-[#0B1B49] transition-colors"
      ...
      <div className="sticky bottom-0 bg-white border-t border-[#E5E7EB] shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
