...
export function AdminBillingLedger() {
  ...
  return <section className="rounded-xl border border-border bg-white p-4 space-y-4">
    ...
    {data && !data.configured && <p className="text-sm text-amber-800">{t('ربط حساب البائع يحتاج إعدادًا.', 'Seller account configuration is required.')}</p>}
    ...
    <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-muted/50 text-xs text-muted-foreground"><tr>{[t('المنشأة والفاتورة', 'Company & invoice'), t('الدفع', 'Payment'), t('التجديد', 'Renewal'), t('المزامنة', 'Sync'), t('المستند', 'Document')].map(x => <th key={x} className="p-3 text-start">{x}</th>)}</tr></thead><tbody>
      {(data?.invoices || []).map((i: any) => { const customer = data.customers.find((x: any) => x.orgId === i.orgId); return <tr key={i.id} className="border-b border-border/60 align-top">
        <td className="p-3"><Link className="text-primary" to={`/admin/orgs/${i.orgId}?tab=subscription`}>{customer?.legalName || i.customerName}</Link><div className="text-xs mt-1"><bdi>{i.number}</bdi> · <bdi>{money(i.totalMinor, i.currency)}</bdi></div>{i.customerName && i.customerName !== customer?.legalName && <div className="text-xs text-amber-800 mt-1">{t('اسم الفاتورة الأصلية', 'Original invoice name')}: {i.customerName}</div>}</td>
        <td className="p-3"><span className={i.status === 'paid' ? 'text-emerald-700' : 'text-amber-800'}>{i.status === 'paid' ? t('مدفوعة', 'Paid') : i.status}</span><div className="text-xs mt-1">{t('المتبقي', 'Remaining')}: <bdi>{money(i.remainingMinor, i.currency)}</bdi></div></td>
        ...
        <td className="p-3 text-xs"><div className={`flex gap-1 items-center ${i.ledgerStatus === 'POSTED' ? 'text-emerald-700' : 'text-amber-800'}`}>{i.ledgerStatus === 'POSTED' ? <CheckCircle2 className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}{i.ledgerStatus === 'POSTED' ? t('الفاتورة محفوظة ومطابقة', 'Invoice saved and matched') : i.ledgerStatus === 'INTERNAL' ? t('اشتراك داخلي للبائع · للمراجعة', 'Seller internal subscription · review') : t('الترحيل يحتاج مراجعة', 'Accounting needs review')}</div>{customer?.identityStatus !== 'MATCHED' && <div className="text-amber-800">{t('هوية الفوترة تحتاج مطابقة', 'Billing identity needs review')}</div>}{i.ledgerError && <div className="mt-1" dir="ltr">{i.ledgerError}</div>}<div className="mt-1 text-muted-foreground"><bdi>{date(i.syncedAt)}</bdi></div></td>
    ...
    {!!data?.events?.length && <p className="text-xs text-amber-800">{t('أحداث Stripe بانتظار المعالجة أو إعادة المحاولة', 'Stripe events awaiting processing or retry')}: {data.events.length}</p>}
  </section>;
}
