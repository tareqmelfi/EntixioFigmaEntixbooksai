  ...
  return <section className="rounded-lg border border-border p-4 space-y-3" aria-labelledby="number-preferences-title">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      <label className="space-y-2 text-xs"><span>{t("لغة الواجهة", "Interface language")}</span>
        <select className="block w-full rounded-md border border-border bg-white px-3 py-2 text-sm" value={language} onChange={e => setLanguage(e.target.value as "ar" | "en")}>
          <option value="ar">العربية</option><option value="en">English</option>
        </select>
      </label>
      <label className="space-y-2 text-xs"><span>{t("نمط الأرقام", "Number style")}</span>
        <select className="block w-full rounded-md border border-border bg-white px-3 py-2 text-sm" value={numberingSystem} onChange={e => setSaveFailed(!setNumberingSystem(e.target.value as NumberingSystem))}>
    ...
    {saveFailed && <p role="alert" className="text-xs text-amber-700">{t("تعذّر حفظ التفضيل. اسمح بالتخزين في المتصفح ثم حاول مجددًا.", "Could not save this preference. Allow browser storage and try again.")}</p>}
  </section>;
}
