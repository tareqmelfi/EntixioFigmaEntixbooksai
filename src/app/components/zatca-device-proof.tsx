...
export function ZatcaDeviceProof({ status }: { status: ZatcaStatus }) {
  ...
  return <section className="space-y-5 rounded-lg border border-border bg-card p-5" aria-label={t("سجل ربط جهاز المنشأة", "Organization device onboarding record")}>
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="flex min-w-0 items-start gap-2.5" aria-live="polite">
        {current ? <ShieldCheck className="mt-0.5 size-5 shrink-0 text-success" strokeWidth={1.75} /> : <CircleAlert className="mt-0.5 size-5 shrink-0 text-warning" strokeWidth={1.75} />}
        <div className="min-w-0">
          <h3 className="text-base font-semibold text-foreground">{title}</h3>
          <p className="mt-0.5 inline-flex items-center gap-1.5 text-xs font-semibold">
            <span className={`ledger-dot ${current ? "text-success" : "text-warning"}`} aria-hidden="true" />
            <span className={current ? "text-success" : "text-warning"}>{current ? t("مرتبط · بيئة الإنتاج", "Bound · production") : t("يحتاج مراجعة", "Needs review")}</span>
          </p>
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-2" aria-label={t("تنزيل سجل الربط", "Download onboarding record")}>
        {previewReady ? <Button asChild variant="outline" size="sm"><a href={preview.pdfUrl} download={`${filename}.pdf`}><FileDown className="me-2 size-4" strokeWidth={1.75} />{t("تنزيل السجل · PDF", "Download record · PDF")}</a></Button> : <Button variant="outline" size="sm" disabled>{t("تجهيز التنزيل…", "Preparing download…")}</Button>}
      </div>
    </div>
    ...
    {previewReady && <a href={preview.url} target="_blank" rel="noopener noreferrer" className="block rounded-lg focus-visible:outline-2 focus-visible:outline-primary" aria-label={t("فتح سجل الربط بالحجم الكامل", "Open the full-size onboarding record")}><img src={preview.url} width={2480} height={1754} alt={t("سجل ربط جهاز المنشأة مع رمز QR ومرجع السجل وبصمة الشهادة", "Device onboarding record with QR, record reference and certificate fingerprint")} className="w-full rounded-lg border border-border bg-background" /></a>}
    {!previewReady && current && !error && <div role="status" className="flex aspect-[297/210] items-center justify-center rounded-lg border border-border bg-background text-sm text-muted-foreground">{t("جارٍ تجهيز سجل الربط…", "Preparing the onboarding record…")}</div>}
    <p className="text-xs text-muted-foreground">{t("آخر تحديث للحالة", "Status last updated")}: <bdi className="font-code">{new Date(proof.checkedAt).toLocaleString(displayLocale("en-GB"), { timeZone: "Asia/Riyadh" })}</bdi> · {t("بتوقيت الرياض", "Riyadh time")}</p>
    <div role="status" className={`rounded-lg border border-border bg-card p-3 text-sm text-foreground border-s-[3px] ${live && !delivery?.needsReview ? "border-s-success" : "border-s-warning"}`}>
    ...
    <details open={!current || undefined} className="space-y-4 text-sm"><summary className="cursor-pointer font-medium text-foreground">{t("بيانات الشهادة ومصدر التحقق", "Certificate details and verification source")}</summary>
    ...
    <p className="text-xs text-muted-foreground">{t("آخر تحقق", "Last checked")}: <bdi className="font-code">{new Date(proof.checkedAt).toLocaleString(displayLocale("en-GB"), { timeZone: "Asia/Riyadh" })}</bdi> · {t("بتوقيت الرياض", "Riyadh time")}</p>
    <p className="break-all text-xs text-muted-foreground">SHA-256: <bdi className="font-code">{cert.fingerprint}</bdi></p>
    <div className="flex flex-wrap items-center gap-2">{previewReady && <Button asChild variant="secondary" size="sm"><a href={preview.url} download={`${filename}.png`}><Download className="me-2 size-4" strokeWidth={1.75} />{t("تنزيل صورة", "Download image")}</a></Button>}<Button variant="outline" size="sm" onClick={invalidateZatcaStatus}>{t("تحديث الحالة", "Refresh status")}</Button><Button asChild variant="secondary" size="sm"><a href={FATOORA_DEVICE_PORTAL} target="_blank" rel="noopener noreferrer">{t("مراجعة الجهاز في بوابة فاتورة", "Review device in Fatoora")}<ExternalLink className="ms-2 size-3.5" strokeWidth={1.75} /></a></Button></div>
    </details>
