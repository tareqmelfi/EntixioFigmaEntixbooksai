...
export function ZatcaDeviceProof({ status }: { status: ZatcaStatus }) {
  ...
  return <section className={`space-y-4 rounded-xl border p-5 ${current ? "border-emerald-200 bg-emerald-50/40" : "border-amber-200 bg-amber-50"}`} aria-label={t("سجل ربط جهاز المنشأة", "Organization device onboarding record")}>
    ...
    {accepted ? <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900" role="status"><p className="font-semibold">{t("تم قبول فاتورة فعلية في بيئة الإنتاج", "An actual production invoice was accepted")}</p><p><bdi>{accepted.invoiceNumber} · {accepted.status}</bdi> · <bdi>{new Date(accepted.acceptedAt).toLocaleString("en-GB", { timeZone: "Asia/Riyadh" })}</bdi></p><p className="break-all text-xs">UUID: <bdi>{accepted.uuid}</bdi></p><p className="mt-1 text-xs">{t("هذا القبول يخص الفاتورة المذكورة. الإرسال الحالي مُدار لكل فاتورة؛ الإرسال التلقائي العام لم يُفعّل بعد.", "This acceptance applies to the named invoice. Submission is currently managed per invoice; general automatic submission is not enabled yet.")}</p></div> : <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">{t("إرسال الفواتير غير مفعّل بعد. يظهر قبول الفاتورة عند استلام رد الهيئة على فاتورة فعلية.", "Invoice submission is not active yet. Invoice acceptance requires an authority response to an actual invoice.")}</p>}
