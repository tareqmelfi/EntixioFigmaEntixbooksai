...
export function ZatcaDeviceProof({ status }: { status: ZatcaStatus }) {
  ...
  return <section className={`space-y-4 rounded-xl border p-5 ${current ? "border-emerald-200 bg-emerald-50/40" : "border-amber-200 bg-amber-50"}`} aria-label={t("سجل ربط جهاز المنشأة", "Organization device onboarding record")}>
    ...
    <p className="text-xs text-muted-foreground">{t("آخر تحديث للحالة", "Status last updated")}: <bdi>{new Date(proof.checkedAt).toLocaleString(displayLocale("en-GB"), { timeZone: "Asia/Riyadh" })}</bdi> · {t("بتوقيت الرياض", "Riyadh time")}</p>
    ...
    {accepted ? <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900" role="status"><p className="font-semibold">{t("تم قبول فاتورة فعلية في بيئة الإنتاج", "An actual production invoice was accepted")}</p><p><bdi>{accepted.invoiceNumber} · {accepted.status}</bdi> · <bdi>{new Date(accepted.acceptedAt).toLocaleString(displayLocale("en-GB"), { timeZone: "Asia/Riyadh" })}</bdi></p><p className="break-all text-xs">UUID: <bdi>{accepted.uuid}</bdi></p><p className="mt-1 text-xs">{t("هذا القبول يخص الفاتورة المذكورة؛ تُعرض نتيجة الهيئة لكل فاتورة بصورة مستقلة.", "This acceptance applies to the named invoice; every invoice has its own authority result.")}</p></div> : <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">{t("لم يُحفظ قبول لفاتورة إنتاجية بعد؛ يظهر هنا عند استلام رد الهيئة.", "No production invoice acceptance is stored yet; it appears here after the authority response.")}</p>}
      <p className="mt-1 text-xs">{t("النطاق الحالي: فواتير المبيعات المحلية بالريال بضريبة 15٪، القياسية والمبسطة، مع خصومات البنود. تُرسل الفواتير الجديدة بعد اعتمادها. الإشعارات الدائنة والمدينة والتصدير والمعفاة تحتاج مسارًا إضافيًا.", "Current scope: domestic SAR sales at 15% VAT, standard and simplified, including line discounts. New invoices are submitted after approval. Credit/debit notes, exports and exempt supplies require an additional workflow.")}</p>
    ...
    <p className="text-xs text-muted-foreground">{t("آخر تحقق", "Last checked")}: <bdi>{new Date(proof.checkedAt).toLocaleString(displayLocale("en-GB"), { timeZone: "Asia/Riyadh" })}</bdi> · {t("بتوقيت الرياض", "Riyadh time")}</p>
