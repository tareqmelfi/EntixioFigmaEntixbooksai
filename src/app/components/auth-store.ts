...
import { bindTabOrgUser, chooseMembership, readLegacyOrgId, readTabOrgId } from '../lib/tab-org-selection'
...
export interface AuthState {
  ...
  organizationError?: 'verification_failed' | 'selection_required'
}
  ...
  async refresh(): Promise<void> {
    try {
      ...
      if (data?.user?.id) {
        ...
        // A validated choice belongs to this tab for its entire lifetime.
        // Defaults and other devices' choices may seed a new tab only.
        const sorted = [...memberships].sort((a: any, b: any) =>
          new Date(a?.createdAt || 0).getTime() - new Date(b?.createdAt || 0).getTime()
        )
        const tabId = readTabOrgId(data.user.id)
        const activeMembership = meKnown ? chooseMembership(sorted, {
          tabId,
          legacyId: readLegacyOrgId(data.user.id),
          defaultId: me?.defaultOrgId || null,
          serverId: me?.selectedOrgId || null,
        }) : null
        const organizationError = !meKnown ? 'verification_failed' as const
          : tabId && !activeMembership && sorted.length ? 'selection_required' as const : undefined
        ...
        if (meKnown) bindTabOrgUser(data.user.id)
        if (activeMembership?.org?.id) setOrgId(activeMembership.org.id)
        else setOrgId(null, false) // Retain the tab's choice while verification is unavailable.
        ...
        this.state = { user: newUser, isAuthenticated: true, loading: false, needsOnboarding, organizationError }
      } else {
        writeCachedUser(null)
        this.state = { user: null, isAuthenticated: false, loading: false }
      }
